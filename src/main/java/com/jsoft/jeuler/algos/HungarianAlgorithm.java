...
public class HungarianAlgorithm {
    ...
    private void setData(int[][] matrix) {
     ...
     */
    private int[][] findOptimalAssignment() {
        step1();    // reduce matrix
    ...
    private int[][] copyMatrixAndNegate(int[][] matrix) {
        int [][] myInt = new int[matrix.length][];
        for(int i = 0; i < matrix.length; i++) {
    ...
    public int[][] execute(int[][] matrix) {
        return execute(matrix, false);
    }

    public int[][] execute(int[][] matrix, boolean calcMaxCost) {
        int [][] copyMatrix = calcMaxCost ? copyMatrixAndNegate(matrix) : copyMatrix(matrix);
        setData(copyMatrix);
        //ha.printResult(matrix, result);
        return findOptimalAssignment();
    }

    private static void printResult(int[][] costMatrix, int[][] assignment) {
        System.out.println("Optimal assignment:");
        if (assignment.length > 0) {
            // print assignment
            for (int i = 0; i < assignment.length; i++) {
                System.out.print("Col" + assignment[i][0] + " => Row" + assignment[i][1] + " (" + costMatrix[assignment[i][0]][assignment[i][1]] + ")");
                System.out.println();
            }
        } else {
    ...
    public static void main(String[] args) {
        ...
        int[][] result = ha.execute(costMatrix);
        printResult(costMatrix, result);
        ...
        result = ha.execute(costMatrix2);
        printResult(costMatrix2, result);
        ...
        result = ha.execute(costMatrix3, true);
        printResult(costMatrix3, result);
    }
}
