...
import java.util.*;
    ...
    public class JEulerProblem_0083 extends EulerSolver {
        ...
        public class Node {
            ...
            public void setRelation(Node l, Node r, Node t, Node b) {
                ...
                if(leftNode != null) {
                    adjacentNodes.put(leftNode, value + leftNode.value);
                }
            }
        ...
        public JEulerProblem_0083(int problemNumber) {
            super(problemNumber);
        }
...
//                    {805, 732, 524, 37, 331}};
            ...
            try {
                matrix = loadData("data/problem83/p083_matrix.txt", LIMIT);
            } catch(Exception e) {
            ...
            Node startNode = sourceGraph.getNode(getKey(0, 0));
            Node targetNode = targetGraph.getNode(getKey(LIMIT-1, LIMIT-1));
            calculateShortestPathFromSource(graph, startNode);

            int shortestValue = 0;
            for (Node n : targetNode.getShortestPath()) {
                shortestValue += n.value;
            }
            shortestValue += targetNode.value;


            System.out.printf("%s - %s with a value of %d\n", startNode.getName(), targetNode.getName(), shortestValue);

            return Integer.toString(shortestValue);
        }
        ...
        public String getProblemStatement() {
            return "https://projecteuler.net/problem=83";
        }
